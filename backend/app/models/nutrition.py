...
class PreworkoutLog(Base):
    __tablename__ = "preworkout_logs"
    ...
    citrulline_mg: Mapped[float | None] = mapped_column(Float, nullable=True)
    beta_alanine_mg: Mapped[float | None] = mapped_column(Float, nullable=True)
    creatine_mg: Mapped[float | None] = mapped_column(Float, nullable=True)
    calories: Mapped[float | None] = mapped_column(Float, nullable=True)
    protein_g: Mapped[float | None] = mapped_column(Float, nullable=True)
    carbs_g: Mapped[float | None] = mapped_column(Float, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())

    __table_args__ = (Index("ix_preworkout_user_date", "user_id", "recorded_date"),)
...
class MealPlanTemplate(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), index=True)
    day_type: Mapped[str] = mapped_column(String(20), nullable=False)  # "training" or "rest"
    phase: Mapped[str] = mapped_column(String(30), nullable=False)
    meals_json: Mapped[dict] = mapped_column(JSONB, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())

    __table_args__ = (Index("ix_meal_plan_user_phase", "user_id", "phase"),)
